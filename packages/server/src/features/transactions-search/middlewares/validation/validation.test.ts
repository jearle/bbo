  ...
  describe('currencyValidator()', () => {
    currencies.forEach((currency: Currency) => {
      it(`should return true for currency ${currency}`, async () => {
        const result = await currencyValidator({ aggregationType: 'PRICE', currency });
        expect(result).toBe(true);
      });

      it(`should return true for lower case currency ${currency.toLowerCase()}`, async () => {
        const result = await currencyValidator({ aggregationType: 'PRICE', currency: currency.toLowerCase()});
        expect(result).toBe(true);
      });
    });
    ...
    it('should reject with an error message for missing required currency', () => {
      expect(
        currencyValidator({ aggregationType: 'PRICE' })
      ).rejects.toEqual('Must supply currency for aggregation type: PRICE');
    });
